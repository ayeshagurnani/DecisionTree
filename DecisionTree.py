...
class Leaf:
    ...
    def __init__(self, rows, id, depth, class_label):
        ...
        #for key in self.predictions.items():
            #print(key)
            #if(key1 > key[0]):
              #  predicted_classlabel = key[0]
             #   print(predicted_classlabel)
            #else:
            #    predicted_classlabel = key[0]
            #    print(predicted_classlabel)
            #temp = key[1]
...
def build_tree(rows, header, depth=0, id=0):
    ...
    true_branch = build_tree(true_rows, header, depth + 1, left_node_id )

    # Recursively build the false branch.
    false_branch = build_tree(false_rows, header, depth + 1, right_node_id)
...
def classify(row, node):
    ...
    if isinstance(node, Leaf):
        ...
        #print("AAAAAAA")
        class_label = [key for m in [max(node.predictions.values())] for key, val in node.predictions.items() if
                       val == m]
        return class_label
...
def getInnerNodes(node, inner_nodes =[]):
